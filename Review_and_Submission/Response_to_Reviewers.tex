...
\toprule
\textbf{Milestone / Variant} & \textbf{Key Architectural Change} & \textbf{Eval $\sigma$} & \textbf{Cover PSNR (dB)} & \textbf{Token MSE} & \textbf{Reverse L2} & \textbf{Rev. PSNR (dB)} \\
\midrule
V3 Baseline & Standard pixel-MSE, pre-serial & $1.24$ & $32.66 \pm 0.54$ & $0.0019$ & $0.0099$ & $20.64$ \\
V4 Serial-Aware & Serial-source consistency ($\mathcal{L}_{\text{ser}}$) & $1.24$ & $32.05 \pm 0.50$ & $0.0016$ & $0.0085$ & $21.31$ \\
V7.1 Capacity Init & Capacity-ceiling penalty ($\mathcal{L}_{\text{cap}}$) & $1.32$ & $30.51 \pm 0.39$ & $0.0192$ & $0.0091$ & $20.95$ \\
V7.3 Freq-Guided & Frequency residual loss ($\mathcal{L}_{\text{freq}}$) & $1.56$ & $29.83 \pm 0.24$ & $0.0115$ & $0.0099$ & $20.62$ \\
V7.4 Pre-Repair & Ceiling + frequency + texture gate & $1.36$ & $38.95 \pm 0.43$ & $0.0141$ & $0.0098$ & $20.50$ \\
\textbf{V8 Full (FGSS)} & 3-stage schedule + oracle distillation & $1.24$ & $\mathbf{39.61 \pm 0.35}$ & $\mathbf{0.0133}$ & $\mathbf{0.0022}$ & $\mathbf{27.44}$ \\
\bottomrule
\end{tabular}%
}
\end{center}
...
\item The \textbf{frequency loss} $\mathcal{L}_{\text{freq}}$ penalises low-frequency perturbations ($\boldsymbol{\Delta}_{\text{LL}}$) and steers residual energy into high-frequency texture bands ($\boldsymbol{\Delta}_{\text{HH}}$) where human visual sensitivity is lowest, suppressing color shifts and structural mottling.
...
\noindent \textbf{Response:} In Sections 3.6 and 4.4, we have added both mathematical clarification and psychovisual grounding:
\begin{enumerate}[leftmargin=*]
\item \textbf{Mathematical invariant (Parseval subband conservation):} PSNR is solely a function of global squared error: $\text{PSNR} = 10 \log_{10}(1 / \|\boldsymbol{\Delta}\|_2^2)$. In the orthonormal Haar wavelet transform, total residual energy is strictly preserved across subbands: $\|\boldsymbol{\Delta}\|_2^2 = \|\boldsymbol{\Delta}_{\text{LL}}\|_2^2 + \|\boldsymbol{\Delta}_{\text{LH}}\|_2^2 + \|\boldsymbol{\Delta}_{\text{HL}}\|_2^2 + \|\boldsymbol{\Delta}_{\text{HH}}\|_2^2$. Therefore, shifting perturbation energy between subbands leaves global MSE (and aggregate PSNR) unchanged by mathematical construction.
\item \textbf{Psychovisual and structural mechanism:} The Human Visual System (HVS) contrast sensitivity function drops rapidly at higher spatial frequencies. By explicitly penalizing the low-pass smooth carrier band $\boldsymbol{\Delta}_{\text{LL}}$ while rewarding placement in the high-frequency band $\boldsymbol{\Delta}_{\text{HH}}$ ($\lambda_{\text{HH}}=0.1$), $\mathcal{L}_{\text{freq}}$ concentrates embedding perturbations into artistic brushstroke textures and edges where they are perceptually masked. Disabling $\mathcal{L}_{\text{freq}}$ leaves aggregate cover PSNR identical at the reported precision ($41.37\pm0.00$\,dB) due to energy conservation, but the spectral reallocation directly suppresses visible banding and color mottling in smooth regions, yielding our low perceptual distortion ($\text{LPIPS}=0.0195$) and high structural similarity ($\text{SSIM}=0.9914$).
\end{enumerate}
...
\item \textbf{Frequency guidance mechanism:} We provided the mathematical justification for PSNR invariance (Parseval subband energy conservation) and demonstrated how penalizing $\boldsymbol{\Delta}_{\text{LL}}$ shifts embedding perturbations into high-frequency texture bands where HVS sensitivity is minimal.
