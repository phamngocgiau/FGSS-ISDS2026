...
\tikzset{
    ...
    drawio badge/.style={
        draw=drawioBlueLine, line width=1pt,
        fill=white, rounded corners=3pt,
        minimum height=1.25cm, minimum width=3.6cm,
        align=center, drop shadow={shadow xshift=1pt, shadow yshift=-1pt, opacity=0.22}
    },
...
% ---- readability upgrades: table spacing + a reusable "takeaway" box ----
...
\textbf{1\textsuperscript{st} Ngoc-Giau Pham} &
\textbf{2\textsuperscript{nd} Phu-Thanh Ha-Quach} &
\textbf{3\textsuperscript{rd *} Hong-Ngoc Tran\thanks{Corresponding author: Hong-Ngoc Tran (e-mail: ngoc.th@vgu.edu.vn).}} \\

\textit{Tra Vinh Univ. \& Tien Giang Univ.} &
\textit{Vietnamese-German University} &
\textit{Vietnamese-German University} \\

Vinh Long \& Dong Thap, Vietnam &
Ho Chi Minh City, Vietnam &
Ho Chi Minh City, Vietnam \\

phamngocgiau@tgu.edu.vn &
thanhbo025@gmail.com &
ngoc.th@vgu.edu.vn \\
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{paper_assets/intro_teaser.png}
...
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{@{}p{0.20\textwidth}p{0.25\textwidth}p{0.27\textwidth}p{0.19\textwidth}@{}}
\toprule
...
\midrule
\multicolumn{4}{@{}l}{\textcolor{drawioBlueLine}{\textbf{FGSS (this work)}}\ \ adds an explicit capacity-ceiling barrier and a semantic-token payload.} \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{paper_assets_old/figure1_architecture.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\columnwidth]{paper_assets/ceiling_penalty.png}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.88\textwidth]{paper_assets/phase_timeline.png}
...
Figure~\ref{fig:style_grid} illustrates the five standard painting styles used as style exemplars throughout the training protocol. The diverse array of brushstrokes, color palettes, and surface textures provided by these styles offers a comprehensive testbed for the encoder's noise-blending capabilities. Figure~\ref{fig:multi_style} confirms the same point from the opposite direction: holding the content image fixed and cycling through all five styles, cover PSNR stays within a narrow $\sim\!40\pm0.6$\,dB band, so the encoder is not secretly tuned to one artistic texture.
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth,height=0.40\textheight,keepaspectratio]{paper_assets/qualitative_grid.jpg}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth,height=0.40\textheight,keepaspectratio]{paper_assets/freq_decomp.jpg}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth,height=0.46\textheight,keepaspectratio]{paper_assets/attack_grid.jpg}
...
To further validate the model's generalizability beyond the training distribution, Table~\ref{tab:cross_dataset} reports quantitative metrics on four completely unseen datasets (COCO, DIV2K, CelebA, BOSSBase) combined with an out-of-distribution artistic style, and Figure~\ref{fig:cross_dataset} shows the corresponding qualitative examples and residuals.
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth,height=0.46\textheight,keepaspectratio]{paper_assets/cross_dataset_grid.jpg}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth,height=0.46\textheight,keepaspectratio]{paper_assets/multi_style_grid.jpg}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{paper_assets/robustness_overview.png}
...
Table~\ref{tab:sota_style} compares our method against the style-transfer steganography family. Because different works measure PSNR against different reference images (pre-style cover, post-style cover, or recovered secret), we state the \emph{metric target} explicitly for each row rather than allow the numbers to imply a false equivalence. Only methods that report concrete numerical results are included; methods with purely qualitative evaluation (Style-Stego-GAN~\cite{sstegogan2024}, Style-DDPM~\cite{sddpm2025}, STIS-GAN~\cite{stisgan2026}) are instead discussed in Section~II. Table~\ref{tab:sota_cross} provides additional context against general deep steganography and coverless baselines.
